...
export default function StudentGradesPage() {
  ...
  async function load() {
    ...
    const rows = (sc.data ?? []) as GradeRow[];
    const assessmentIds = Array.from(new Set(rows.map(r => r.assessment_id)));

    // assessments
    const a = assessmentIds.length
      ? await supabase.from("assessments").select("id,title,max_score,date,subject_id").in("id", assessmentIds)
      : { data: [], error: null };
    ...
    const merged = rows.map((r) => {
      ...
      const percent = scoreNum != null && max ? Math.round((scoreNum / max) * 100) : null;
    ...
    // stats
    const numeric = merged.filter((x) => typeof x.score === "number" && typeof x.max_score === "number" && x.max_score);
  ...
  const filtered = grades.filter((x) => {
    ...
    if (!t) return true;
    return (x.subject ?? "").toLowerCase().includes(t) || (x.assessment ?? "").toLowerCase().includes(t);
  });

  return (
    <AppShell>
      <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <PageHeader title="درجاتي" subtitle="عرض درجات الاختبارات والواجبات لكل المواد" />
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-4">
        <KpiCard label="عدد الدرجات" value={loading ? "—" : String(stats.count)} />
        <KpiCard label="المتوسط العام" value={loading ? "—" : `${stats.avg}%`} hint="محسوب كنسبة من الدرجة النهائية" />
        <KpiCard label="أفضل نتيجة" value={loading ? "—" : `${stats.best}%`} />
      </div>

      <Card className="rounded-2xl mb-4">
        <CardContent className="p-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <Input className="max-w-md" placeholder="ابحث بالمادة أو اسم الاختبار..." value={q} onChange={(e) => setQ(e.target.value)} />
            ...
            <Table>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow><TableCell colSpan={6} className="py-10 text-center text-slate-500">لا توجد درجات</TableCell></TableRow>
                ) : (
                  filtered.map((g) => (
                    <TableRow key={g.id}>
                      ...
                      <TableCell dir="ltr">
                        {g.score ?? "—"} {g.max_score ? <span className="text-slate-400">/ {g.max_score}</span> : null}
                      </TableCell>
                      <TableCell>{g.percent == null ? "—" : <Badge className="bg-slate-900">{g.percent}%</Badge>}</TableCell>
